...
def index():
    posts = [
        {
            'author': {'username': 'Grzegorz'},
            'body': 'skibidi'
        },
        {
            'author': {'username': 'Hades'},
            'body': 'dupa cyce waodice'
        }
    ]
    return render_template('index.html', title='Strona Glowna', posts = posts)
...
def register():
    ...
    if form.validate_on_submit():
        user = User(nazwa=form.nazwa.data, email=form.email.data)
...
def edit_profile():
    ...
    if form.validate_on_submit():
        current_user.nazwa = form.username.data
        current_user.about_me = form.about_me.data
        db.session.commit()
        flash('Your changes have been saved.')
        return redirect(url_for('edit_profile'))
    elif request.method == 'GET':
        form.username.data = current_user.nazwa
